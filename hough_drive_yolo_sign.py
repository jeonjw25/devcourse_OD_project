...
def get_stop_line(frame):
    ...
    # blur
    kernel_size = 5
    blur_roi = cv2.GaussianBlur(roi,(kernel_size, kernel_size), 0)

    if cv2.mean(blur_roi) < 50:# TODO: varify this
        return True
...
def get_stop_sign(frame):
    ...
    # TODO: get red sign in traffic sign
...
def drive_normal(direction=None, time=1.0):
    print("drive_normal")
    global pub
    global image
    global sleep_rate
    global Width, Height
    global xycar_motor_msg, steer_mm
    global prev_angle, steering_gain
    ...
    while cnt < max_cnt:
        ...
        steer_angle = steering_gain * get_steer_angle(position)
...
def drive_left():
    print("drive_left")
    drive_normal(direction="left", time=3.0)


def drive_right():
    print("drive_right")
    drive_normal(direction="right", time=3.0)


def drive_stop(time=5.0):
    print("drive_stop")
    global pub
    global image
    global sleep_rate
    ...
    cnt = 0
    if time == 1.0:
        max_cnt = 1# only one loop, it will 1/sleep_rate seconds
    else:
        max_cnt = sleep_rate * time# stop 'time' seconds

    while cnt < max_cnt:
        while not image.size == (640*480*3):
            continue

        if get_stop_line(image):
...
def find_traffic_light(time=5.0):
...
def start():
    ...
    rospy.Subscriber('/yolov3_trt_ros/detections', BoundingBoxes, yolo_trt_callback, queue_size=1)
    print "---------- Xycar A2 v1.0 ----------"
